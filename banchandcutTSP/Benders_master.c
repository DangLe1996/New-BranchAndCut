...
int create_CPLEX_master_lp(CPXENVptr env, CPXLPptr *lp_ptr, GLOBAL_INFO global) {
	...
	*lp_ptr = CPXcreateprob(env, &status, "MASTER_PROBLEM");
	...
	for (e = 0; e<E; e++){
		pos_y[e] = index1;
		obj[index1] = F[index_i[e]][index_j[e]];
		lb[index1] = 0;
		ub[index1] = 1;
		sprintf(colname[index1], "y_%d,%d ", index_i[e], index_j[e]);
		index1++;
	}
	...
	for (k = 0; k<K; k++){
		pos_z[k] = E + index1;
		obj[index1] = 0;// Com[k].w;
		lb[index1] = 0;
		ub[index1] = CPX_INFBOUND;
		sprintf(colname[index1], "z_%d", k);
		index1++;
	}
	status = CPXnewcols(env, *lp_ptr, index1, obj, lb, ub, NULL, colname);
	if (status)
	...
	for (i = 0; i < N; i++){
		...
		matbeg[index1++] = index;
		for (e = 0; e < E; e++){
			if (index_i[e] == i || index_j[e] == i){
				matind[index] = pos_y[e];
				matval[index++] = 1;
			}
		}
	}
	status = CPXaddrows(env, *lp_ptr, 0, index1, index, rhs, sense, matbeg, matind, matval, NULL, NULL);
	if (status)
...
int solve_root_node(CPXENVptr env, CPXLPptr lp, GLOBAL_INFO global) {
	...
	int		E = global.data->E;
	int		K = global.data->K;
	struct timespec inicio_iter,fin_iter;
	double elapsediter;
	//	global.param		= &param;
	//double	*relax = create_double_vector(E + K);
	int i,e;
	int Fcuts ;
	//global.param		= &param;
	//global.var			= &var;
	CPXENVptr env2 = NULL;     // pointer to Cplex Enviroment
	CPXLPptr  lp2 = NULL;     // pointer to Cplex Linear Program data structure	
	
	if (global.param->SCREEN_OUTPUT >= 2) { printf("\t\tNumRows: %d\n", CPXgetnumrows(env, lp)); }

	// Solve (Linear-Relaxed) Master Problem
	start = clock();
	CPXwriteprob(env, lp, "model_principal.lp", NULL);
	status = CPXlpopt(env, lp);
	assert(status == 0);
	global.results->master_time += elapsed(start);
	///Luisa 24-3-19
	CPXgetobjval(env, lp, &(global.results->LowerBound));
	printf("LOWER BOUND %f", global.results->LowerBound);
	global.results->initialrel = global.results->LowerBound;

	global.results->relax_solution = create_double_vector(E + K);
	status = CPXgetx(env, lp, global.results->relax_solution, 0, CPXgetnumcols(env, lp) - 1);
	...
	// while we are improving 'enough': COMENTADO [ESTABA EN EL PROGRAMA INICIAL]
	...
	Fcuts=1;  //Fcuts=1 porque suponemos que inicialmente se va a meter algun corte
	
	
	
	//-----Generate empty lp2--------------------------------------------------------------------------// [PARA CREAR EL PROBLEMA DEL PASO 1
	global.results->alpha_cur = create_double_vector(E);// Create vector  of alpha coefficients
		global.results->const_record= create_int_vector(E);// Create vector  to control de variables appearing in the constraints of lp2

	if (elapsed(global.var->init_time) < global.param->MAX_CPU_TIME) {
		status = create_CPLEX_Step1_enviroment(&env2,global);
		assert(status == 0);
	}
	
	// Generate the initial LP for Step1 in Fenchel procedure:
   if (elapsed(global.var->init_time) < global.param->MAX_CPU_TIME) {
		if (global.param->SCREEN_OUTPUT >= 1) { printf("\nGENERATING BASE MODEL FOR STEP 1 OF FENCHEL CUTS:\n"); }
			status = create_CPLEX_Step1_lp(env2, &lp2, global);
			assert(status == 0);
	}
	...
	while(Fcuts >0){
		...
		///SEC constraints///
		clock_gettime(CLOCK_MONOTONIC, &inicio_iter);
		...
		//getchar();
		//LUISA
		global.results->Bfeascuts += cuts_found;
		printf("cuts found %d \n", cuts_found);
		CPXwriteprob(env, lp, "model_principal.lp", NULL);
		...
		if (cuts_found == 0) {
			...
			//Obtain a fenchel cut
			if (elapsed(global.var->init_time) < global.param->MAX_CPU_TIME) {
				if (global.param->SCREEN_OUTPUT >= 1) { printf("\n STARTING FENCHEL PROCEDURE\n"); }
				Fcuts = solve_Fenchel_procedure(global,env2,lp2);
			}
			...
			printf("Final Fenchel cut:\n");
			for ( e = 0; e < E; e++){
				printf("%f ", global.results->alpha_cur[e]);
			}
			printf("\n");

			global.results->NFcuts += 1;
			//ncutsFenchel += 1;
			////Resolver el problema con los cortes de Fenchel
			//Añadir cortes de Fenchel
			add_Fenchel_cut(global, env, lp);
		...
		start = clock();
		CPXwriteprob(env, lp, "model_principal_eil51_sin_reduccion.lp", NULL);
		status = CPXlpopt(env, lp);
		assert(status == 0);
		global.results->master_time += elapsed(start);
		//Luisa//		
	...
	/*	for (i = 0; i < CPXgetnumcols(env, lp); i++)
			printf("%f ", global.results->relax_solution[i]);*/
		...
		global.results->final_gap = (global.results->UpperBound-global.results->LowerBound) / global.results->UpperBound;
		if (global.param->SCREEN_OUTPUT >= 1) { printf("Root\t\t\t%.2f <- %.2f%% -> %.2f  \tTime: %.2f\"\n", global.results->LowerBound, 100*global.results->final_gap, global.results->UpperBound, elapsed(global.var->init_time)); }
	    FILE *output_prueba = NULL;
	    
	    clock_gettime(CLOCK_MONOTONIC, &fin_iter);
		elapsediter = (fin_iter.tv_sec - inicio_iter.tv_sec);
	    elapsediter += (fin_iter.tv_nsec - inicio_iter.tv_nsec) / 1000000000.0;
	    
	    
	     output_prueba = open_file("cuts_evolution.txt", "a");
	      fprintf(output_prueba,"SEC CUTS %d, Fenchel cuts %d Lower Bound %.2f  Tiempo iter %.2f Numb restricciones %d \n", global.results->Bfeascuts, global.results->NFcuts, global.results->LowerBound,elapsediter,global.results->numb_rest);
	    fclose(output_prueba);
	   // getchar();
...
int CPXPUBLIC lazy_cut_callback(CPXCENVptr env, void *cbdata, int wherefrom, void *cbhandle, int *useraction_p) {
	...
	if (elapsed(global.var->init_time) < global.param->MAX_CPU_TIME) {
		...
		if(cuts_found == 0){
			...
			for ( e = 0; e < E; e++){
				obj_value += global.data->F[global.data->index_i[e]][global.data->index_j[e]] * global.var->capacity[global.data->index_i[e]][global.data->index_j[e]];
			}
